...
def _set_gpu_runtime_env() -> None:
    if config.is_fbcode():
        ...
        if torch.version.hip is None:
            # For cuda
            if (
                "CUDA_HOME" not in os.environ
                and "CUDA_PATH" not in os.environ
            ):
                os.environ["CUDA_HOME"] = os.path.dirname(build_paths.cuda())
        else:
            # For rocm, we will fallback to deduce from hipcc dir.
            pass
    else:
        # No special treatment needed for OSS
        pass
...
def get_include_and_linking_paths(
    ...
    if config.is_fbcode():
        ...
        # we need to put each entire -I list despite the redundancies
        if  torch.version.hip is not None:
    ...
    if aot_mode and cuda and config.is_fbcode():
        ...
        if torch.version.hip is not None:
            static_link_libs = ["-Wl,-Bstatic", "-Wl,-Bdynamic"]
        else:
